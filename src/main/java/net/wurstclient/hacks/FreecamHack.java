	...
	IsNormalCubeListener, SetOpaqueCubeListener, RenderListener
{
	...
	public void onRender(MatrixStack matrixStack, float partialTicks)
	{
		...
		Vec3d center = fakePlayer.getBoundingBox().getCenter();
		
		double extraSize = 0.05;
		Box box = fakePlayer.getBoundingBox().offset(0, extraSize, 0)
			.expand(extraSize);
		RenderUtils.drawOutlinedBox(matrixStack, box, colorI, true);
		
		if(RenderUtils.hasLineOfSight(center))
			RenderUtils.drawTracer(matrixStack, partialTicks, center, colorI,
				true);
	}
}
