...
public enum RenderUtils
{
	...
	public static boolean hasLineOfSight(Vec3d target)
	{
		if(target == null)
			return false;
		
		if(WurstClient.MC.world == null
			|| WurstClient.MC.gameRenderer.getCamera() == null)
			return true;
		
		Vec3d start = getCameraPos();
		if(target.squaredDistanceTo(start) < 1e-6)
			return true;
		...
		double targetDistSq = target.squaredDistanceTo(start);
		return hitDistSq >= targetDistSq - 1e-4;
	}
